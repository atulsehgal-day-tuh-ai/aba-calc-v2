...
export async function seedDatabase(db: DbClient) {
  ...
  const countResult = await db.execute('SELECT COUNT(*) as c FROM patients');
  ...
  for (const p of patients) {
    await db.execute({
      ...
      args: [p.id, p.name, p.age, p.diagnosis, p.educational_setting, p.living_situation],
    });
  }
  ...
  for (const p of profiles) {
    await db.execute({
      sql: `INSERT INTO payer_profiles (id, name, max_hours, min_hours, fii_w, vin_w, vb_w, beh_w, env_w,
        age_mult_young, age_mult_mid, age_mult_teen, sup_pct, pt_range_min, pt_range_max) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [p.id, p.name, p.max_hours, p.min_hours, p.fii_w, p.vin_w, p.vb_w, p.beh_w, p.env_w,
  ...
  const claims = [
    {
      ...
      approved_hours: null, review_notes: null,
    },
    {
    ...
    },
    {
      ...
      status: 'approved', recommended_hours: 20, tier: 2,
      ...
      approved_hours: 20, review_notes: 'Meets medical necessity criteria. Approved as requested.',
    },
    {
      ...
      approved_hours: null, review_notes: 'Insufficient functional impairment documented for requested hours.',
    },
  ];

  for (const c of claims) {
    await db.execute({
      sql: `INSERT INTO claims (id, patient_id, patient_name, age, diagnosis, status, assessment_data, calc_result, ml_prediction, recommended_hours, approved_hours, tier, review_notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [c.id, c.patient_id, c.patient_name, c.age, c.diagnosis, c.status,
        c.assessment_data, c.calc_result, c.ml_prediction, c.recommended_hours,
        c.approved_hours ?? null, c.tier, c.review_notes ?? null],
    });
  }
